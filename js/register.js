...
//validar telefono
function esTlfValido() {
    var tlf = document.getElementById("tlf").value;
    let phonePattern = /^\d{9}|\d{3} \d{3} \d{3}|\d{3} \d{2} \d{2} \d{2}/
    if (!tlf.match(phonePattern)) {
...
function esIbanValido() {
    ...
    if (!iban.match(IBAN_REGEX)) {
        ...
        entidadBancaria = iban.substring(4, 8);
        var swift = genera_swift(entidadBancaria);
